...
{
    std::vector<std::string> in = {par().q, par().qRev, par().qSrc, par().action};
...
void TWardIdentity<FImpl>::execute(void)
{
    ...
    {
        act.ContractConservedCurrent(qRev, q, tmp, qSrc, Current::Vector, mu);
        tmp -= Cshift(tmp, mu, -1);
        vector_WI += tmp;
    }
    ...
    if (par().test_axial)
    {
        ...
        for (unsigned int mu = 0; mu < Nd; ++mu)
        {
            act.ContractConservedCurrent(qRev, q, tmp, qSrc, Current::Axial, mu);
            tmp -= Cshift(tmp, mu, -1);
